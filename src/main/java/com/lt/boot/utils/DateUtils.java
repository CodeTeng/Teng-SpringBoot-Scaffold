...
public class DateUtils extends LocalDateTimeUtil {
    ...
    public static List<String> last15Day() {
        ...
        for (int count = 0; count < 15; count++) {
            // 3.1.格式化时间
            days.add(String.format("%s.%s", repair0(time.getMonthValue(), 2), repair0(time.getDayOfMonth(), 2)));
            // 3.2.日期加1天
     ...
     * @return 字符串
     */
    private static String repair0(int originNumber, int digit) {
